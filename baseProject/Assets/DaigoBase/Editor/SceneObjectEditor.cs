...
public class SceneObjectEditor:PropertyDrawer
{
    protected SceneAsset GetSceneObject(string sceneObjectName)
    {
        ...
        Debug.Log("Scene [" + sceneObjectName + "] cannot be used. Add this scene to the 'Scene is the Build' in the build settings.");
        return null;
    }
    ...
    public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
    {
        ...
        else
        {
            if (newScene.name != property.FindPropertyRelative("m_SceneName").stringValue)
            {
                ...
                if (scnObj == null)
                {
                    Debug.LogWarning("The scene " + newScene.name + " cannot be used. To use this scene add it to the build setting for the project.");
                }
                else
                {
                    var prop = property.FindPropertyRelative("m_SceneName");
